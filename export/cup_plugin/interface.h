...
public:
    ...
    virtual Result<std::string, std::string> gen_cmake(const CMakeContext &context, bool is_dependency) = 0;
    /// @brief The return value of this interface will be used as the file content before the project command in the CMake file.
    /// @param context Context data.
    /// @param is_dependency Indicate whether it is used as a dependency when calling the generation of CMake logic.
    /// @return CMakeLists.txt content.
    virtual Result<std::string, std::string> gen_cmake_global(const CMakeContext &context, bool is_dependency) { return Ok<std::string, std::string>(""); }
    ...
    virtual Result<int, std::string> execute(const cmd::Args &command)
    {
        using namespace std::string_literals;
        return Err<int>("This plugin does not support command execution."s);
    };
};
